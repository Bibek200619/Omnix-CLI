...
from omnix_cli.agents.backend.context_builder import BackendContextBuilder
from omnix_cli.agents.backend.models import BackendAgentResult
from omnix_cli.agents.backend.prompts import (
    build_backend_system_prompt,
    build_backend_user_prompt,
)
...
class BackendAgent:
    """Worker agent that generates backend artifacts."""
    ...
    def __init__(
        ...
        *,
        ...
        context_builder: BackendContextBuilder | None = None,
        provider_registry: ProviderRegistry | None = None,
        settings: Settings | None = None,
    ) -> None:
        self.state_manager = state_manager
        self.memory_manager = memory_manager or MemoryManager(state_manager)
        self.context_builder = context_builder or BackendContextBuilder(
            state_manager,
            self.memory_manager,
        )
    ...
    async def execute_task(self, task: TaskDefinition) -> BackendAgentResult:
        """Execute a backend task and produce an artifact."""

        if task.assigned_agent.value != AgentRole.BACKEND.value:
            msg = f"Task {task.id} is not assigned to the Backend Agent."
        ...
        response = await provider.generate(
            build_backend_user_prompt(context, task_description),
            system_prompt=build_backend_system_prompt(),
            temperature=0.2,
        )
        ...
        artifact_type = cast(
            ArtifactType,
            artifact_data.get("artifact_type", ArtifactType.BACKEND_SERVICE),
        )
    ...
    def _create_provider(self) -> BaseProvider:
        ...
        assignment = models.assignment_for(AgentRole.BACKEND)
        
        if assignment is None or assignment.provider is None or assignment.model is None:
            msg = (
                "Role 'backend' is missing provider/model configuration. "
                "Use `omnix config --set backend=provider:model` first."
            )
    ...
    def _parse_artifact_response(self, response: str) -> dict[str, Any]:
        ...
        if not normalized_response:
            msg = "Backend provider returned an empty response."
        ...
        if not isinstance(payload, dict):
            msg = "Backend provider response must be a JSON object."
            raise ProviderRequestError(msg)
    ...
    def _load_fenced_json_payload(self, response: str) -> Any:
        ...
        if start_idx == -1:
             ...
             if first_brace != -1 and last_brace != -1:
                 try:
                     ...
                     pass
             msg = "Backend provider returned non-JSON output."
             raise ProviderRequestError(msg)
        ...
        if end_idx == -1:
            msg = "Backend provider returned malformed fenced JSON."
            raise ProviderRequestError(msg)
        ...
        try:
            ...
            msg = "Backend provider returned invalid JSON within fences."
