...
from omnix_cli.agents.frontend.context_builder import FrontendContextBuilder
from omnix_cli.agents.frontend.models import FrontendAgentResult
from omnix_cli.agents.frontend.prompts import (
    build_frontend_system_prompt,
    build_frontend_user_prompt,
)
...
class FrontendAgent:
    """Worker agent that generates frontend artifacts."""
    ...
    def __init__(
        ...
        *,
        ...
        context_builder: FrontendContextBuilder | None = None,
        provider_registry: ProviderRegistry | None = None,
        settings: Settings | None = None,
    ) -> None:
        self.state_manager = state_manager
        self.memory_manager = memory_manager or MemoryManager(state_manager)
        self.context_builder = context_builder or FrontendContextBuilder(
            state_manager,
            self.memory_manager,
        )
    ...
    async def execute_task(self, task: TaskDefinition) -> FrontendAgentResult:
        """Execute a frontend task and produce an artifact."""

        if task.assigned_agent.value != AgentRole.FRONTEND.value:
            msg = f"Task {task.id} is not assigned to the Frontend Agent."
        ...
        response = await provider.generate(
            build_frontend_user_prompt(context, task_description),
            system_prompt=build_frontend_system_prompt(),
            temperature=0.2,
        )
        ...
        artifact_type = cast(
            ArtifactType,
            artifact_data.get("artifact_type", ArtifactType.FRONTEND_COMPONENT),
        )
    ...
    def _create_provider(self) -> BaseProvider:
        ...
        assignment = models.assignment_for(AgentRole.FRONTEND)
        
        # Fallback to a default if frontend is not specifically configured?
        # The prompt says: "Invalid artifacts must never be saved." 
        # but doesn't explicitly say what to do if config is missing.
        # Usually we want it configured.
        if assignment is None or assignment.provider is None or assignment.model is None:
            # Maybe check if architect or master is configured and use that as fallback?
            # For now, stick to explicit config or error like PlannerAgent.
            msg = (
                "Role 'frontend' is missing provider/model configuration. "
                "Use `omnix config --set frontend=provider:model` first."
            )
    ...
    def _parse_artifact_response(self, response: str) -> dict[str, Any]:
        ...
        if not normalized_response:
            msg = "Frontend provider returned an empty response."
        ...
        if not isinstance(payload, dict):
            msg = "Frontend provider response must be a JSON object."
            raise ProviderRequestError(msg)
    ...
    def _load_fenced_json_payload(self, response: str) -> Any:
        ...
        # Find first line with ```json or just ```
        start_idx = -1
        ...
        if start_idx == -1:
             ...
             if first_brace != -1 and last_brace != -1:
                 try:
                     ...
                     pass
             msg = "Frontend provider returned non-JSON output."
             raise ProviderRequestError(msg)
        ...
        if end_idx == -1:
            msg = "Frontend provider returned malformed fenced JSON."
            raise ProviderRequestError(msg)
        ...
        try:
            ...
            msg = "Frontend provider returned invalid JSON within fences."
